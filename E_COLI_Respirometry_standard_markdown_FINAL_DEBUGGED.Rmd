...
data <- read_csv("EXP3B_3_09_05_22_e_coli_Oxygen.csv", skip=12, na=c("No Sensor","NA"))
...
data <- data[,1:27] #sometimes you have to change this last number, check it out
...
getwd()
resp_info <- read.csv("template_resp_EXP3B_3_09_05_22_e_coli_Oxygen.csv", header=TRUE) # NUMBER ORDER, NOT LETTER ORDER
...
```{r generate background respiration rate, SW + E. coli}

#backgroun SW rate of SW only
library(respR)
...
SW.EC.data <- na.omit(data[c(29, 4)]) #time.elapsed.min and the column with SW + E_coli
...
write.csv(output_temp, "OUTPUT_EXP3B_3_09_05_22_e_coli_Oxygen.csv", row.names=FALSE)
